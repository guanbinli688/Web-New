...
def _compact_signal_text(value, *, max_chars: int, ticker: str = "") -> str:
    """Compress radar copy into a single scannable Chinese sentence."""
    text = " ".join(str(value or "").split()).strip()
    text = re.sub(r"^(?:触发条件|失效风险|失效|风险)[:：]\s*", "", text)
    if ticker:
        text = re.sub(rf"^{re.escape(ticker)}[:：]?\s*", "", text, flags=re.I)
    ...
    available = max(max_chars - len(prefix), 8)
    if len(text) > available:
        clauses = [part for part in re.split(r"(?<=[；。！？])", text) if part]
        chosen = ""
        for clause in clauses:
            if len(chosen) + len(clause) > available:
                break
            chosen += clause
        if not chosen:
            chosen = text[: max(available - 1, 1)].rstrip("，；。！？、 ") + "。"
        text = chosen
    return prefix + text
...
_MAJOR_CALENDAR_KEYWORDS = (
    ...
    "initial jobless claims", "weekly claims", "real earnings",
    "import and export price", "import/export price",
    "job openings and labor turnover", "jolts",
    "retail sales", "industrial production", "housing starts",
    "building permits", "existing home sales", "consumer credit",
    "quarterly financial report", "quarterly services survey",
    "wholesale trade", "business formation statistics",
    "business inventories", "manufacturing and trade",
    "empire state manufacturing", "business leaders survey",
    "philadelphia fed manufacturing", "survey of consumer expectations",
...
_MAJOR_COMPANY_EARNINGS = {
    ...
    "Walmart IR": {
        "title": "沃尔玛财报",
        "url": "https://stock.walmart.com/events-and-presentations/default.aspx",
    },
    "Costco IR": {
        "title": "开市客财报",
    ...
    "ExxonMobil IR": {
        "title": "埃克森美孚财报",
        "url": "https://corporate.exxonmobil.com/investors/investor-relations/events-and-presentations",
    },
    "Eli Lilly IR": {
        "title": "礼来财报",
        "url": "https://investor.lilly.com/events-and-presentations",
    },
    "UnitedHealth IR": {
        "title": "联合健康财报",
    ...
    "HubSpot IR": {
        "company": "HubSpot",
        "title": "HubSpot财报",
        "url": "https://ir.hubspot.com/",
    },
    "Lennar IR": {
        "company": "Lennar",
        "title": "Lennar财报",
        "url": "https://investors.lennar.com/",
    },
}
...
_COMPANY_EVENT_WORDS = _EARNINGS_WORDS + (
    "investor day", "analyst day", "investor conference",
    "technology conference", "financial community",
)
...
def _calendar_local_importance(title: str) -> int:
    ...
    if "personal income and outlays" in value or "pce" in value:
        return 100
    if "fomc" in value or "联邦公开市场委员会" in value:
        return 99
    ...
    if "lululemon" in value:
        return 88
    if "财报" in value or "业绩发布" in value:
        # Only curated company IR pages reach this scorer, so a named earnings
        # release remains material even without a hand-tuned company score.
        return 88
    if "consumer price index" in value or re.search(r"\bcpi\b", value):
        return 96
    if "retail sales" in value or "零售销售" in value:
        return 92
    if "industrial production" in value or "工业产出" in value:
        return 88
    if "housing starts" in value or "新屋开工" in value:
        return 87
    if "consumer credit" in value or "消费者信贷" in value:
        return 76
    if "initial claims" in value or "首次申请失业" in value:
        return 84
    if "empire state manufacturing" in value or "纽约联储制造业" in value:
        return 83
    if "philadelphia fed manufacturing" in value or "费城联储制造业" in value:
        return 82
    if "survey of consumer expectations" in value or "消费者预期调查" in value:
        return 78
    if "business leaders survey" in value or "服务业活动调查" in value:
        return 72
    if "business inventories" in value or "商业库存" in value:
        return 75
    if "quarterly financial report" in value or "企业财务报告" in value:
        return 66
    if "quarterly services survey" in value or "季度服务业调查" in value:
        return 65
    if "wholesale trade" in value or "批发销售与库存" in value:
        return 64
    if "business formation statistics" in value or "企业成立统计" in value:
        return 60
    if "market holiday" in value or "休市" in value:
        return 85
    ...
    if "producer price index" in value or re.search(r"\bppi\b", value):
        return 87
    if ("treasury" in value and "auction" in value) or "国债拍卖" in value:
        return 82
...
def _specific_company_calendar_title(title: str, source: str) -> str:
    """Turn an IR-backed event into a short title that names the company."""
    value = " ".join(str(title or "").split()).strip()
    source_name = " ".join(str(source or "").split()).strip()
    ticker = COMPANY_SYMBOLS.get(source_name)
    meta = _MAJOR_COMPANY_EARNINGS.get(source_name, {})
    company = str(meta.get("company") or "").strip()
    if not company and ticker:
        company = _company_display_name(ticker)
    if not company:
        return value
    lowered = value.lower()
    already_named = (
    ...
    if already_named and re.search(r"[\u4e00-\u9fff]", value):
        return value

    if any(word in lowered for word in _EARNINGS_WORDS):
        return f"{company}财报"
    if "investor" in lowered or "投资者" in value:
        return f"{company}投资者活动"
    if "guidance" in lowered or "指引" in value:
        return f"{company}业绩指引更新"
    if value and not _is_generic_calendar_title(value):
...
def _parse_major_company_earnings(name: str, html_text: str, *, start: date, days: int = 14):
    ...
    found = []
    company = str(meta.get("company") or meta["title"].removesuffix("财报")).strip()

    def event_title(value: str) -> str:
        lowered = value.lower()
        if any(word in lowered for word in _EARNINGS_WORDS):
            return meta["title"]
        if "analyst day" in lowered:
            return f"{company}分析师日"
        if "investor day" in lowered:
            return f"{company}投资者日"
        return f"{company}投资者会议"
    ...
    # Look only around lines that clearly describe earnings/results or a
    # scheduled investor event. The company name is always retained.
    for i, line in enumerate(lines):
        low = line.lower()
        if not any(word in low for word in _COMPANY_EVENT_WORDS):
            continue
    ...
    # One company may have both a conference and a results call in the window.
    return deduped[:2]
...
def _parse_dedicated_calendar_events(name: str, html_text: str, *, start: date, days: int = 14):
    ...
    if name.startswith("Federal Reserve"):
        ...
        if month_year:
            ...
            # The monthly Fed calendar contains many minor series. Keep only
            # the releases that regularly move rates or broad risk assets.
            rate_sensitive_releases = {
                "g.19 - consumer credit": "美联储消费者信贷",
                "g.17 - industrial production": "美国工业产出与产能利用率",
            }
            for i, line in enumerate(lines):
                normalized_line = line.strip().lower()
                title = next(
                    (label for marker, label in rate_sensitive_releases.items() if marker in normalized_line),
                    "",
                )
                if not title:
                    continue
                for candidate in lines[i + 1:i + 9]:
                    if not re.fullmatch(r"\d{1,2}", candidate.strip()):
                        continue
                    try:
                        release_date = date(year, month, int(candidate.strip()))
                    except ValueError:
                        break
                    add(release_date, title, "Federal Reserve")
                    break
    ...
    if name == "US Census Calendar":
        for row in soup.find_all("tr"):
            ...
            if "new residential sales" in normalized:
                title = "美国新屋销售"
            elif "new residential construction" in normalized or "housing starts" in normalized:
                title = "美国新屋开工与建筑许可"
            elif "advance monthly sales for retail and food services" in normalized:
                title = "美国零售销售"
                ...
                title = "美国贸易帐"
            elif "quarterly financial report" in normalized:
                title = "美国企业财务报告"
            elif "quarterly services survey" in normalized:
                title = "美国季度服务业调查"
            elif "monthly wholesale trade" in normalized:
                title = "美国批发销售与库存"
            elif "business formation statistics" in normalized:
                title = "美国企业成立统计"
            elif "manufacturing and trade" in normalized and "inventories" in normalized:
                title = "美国商业库存"
            else:
                continue
            add(event_date, title, "US Census")

    # New York Fed economic-indicator calendar. Each table cell begins with
    # the day number and contains one or more named releases. It cross-checks
    # important releases that agency-specific parsers can otherwise miss.
    if name.startswith("New York Fed Indicators"):
        month_year = re.search(
            r"\b(" + "|".join(m.title() for m in _MONTH_NUM) + r")\s+(\d{4})\b",
            plain,
            re.I,
        ...
        if month_year:
            ...
            year = int(month_year.group(2))
            release_names = {
                "Survey of Consumer Expectations": "纽约联储消费者预期调查",
                "Initial Claims": "美国首次申请失业救济",
                "NAR Existing Home Sales": "美国成屋销售",
                "Michigan Consumer Survey (Preliminary)": "密歇根消费者信心初值",
                "Empire State Manufacturing Survey": "纽约联储制造业指数",
                "Business Leaders Survey": "纽约联储服务业活动调查",
                "Business Inventories": "美国商业库存",
                "Philadelphia Fed Manufacturing Survey": "费城联储制造业指数",
            }
            for cell in soup.find_all("td"):
                cell_text = " ".join(cell.stripped_strings)
                day_match = re.match(r"\s*(\d{1,2})\b", cell_text)
                if not day_match:
                    continue
                try:
                    ...
                    continue
                for raw_name, title in release_names.items():
                    if raw_name.lower() in cell_text.lower():
                        add(event_date, title, name)

    # NYSE holiday table has a named holiday column followed by year columns.
    if name == "NYSE Holidays":
        for table in soup.find_all("table"):
            year_index = None
            for row in table.find_all("tr"):
                cells = [" ".join(cell.stripped_strings) for cell in row.find_all(["th", "td"])]
                if str(start.year) in cells:
                    year_index = cells.index(str(start.year))
                    break
            if year_index is None:
                continue
            for row in table.find_all("tr"):
                cells = [" ".join(cell.stripped_strings) for cell in row.find_all(["th", "td"])]
                if len(cells) <= year_index or not cells:
                    continue
                holiday = cells[0].strip()
                date_match = re.search(
                    r"\b(" + "|".join(m.title() for m in _MONTH_NUM) + r")\s+(\d{1,2})\b",
                    cells[year_index],
                    re.I,
                )
                if not date_match:
                ...
                month = _MONTH_NUM[date_match.group(1).lower()]
                try:
                    holiday_date = date(start.year, month, int(date_match.group(2)))
                except ValueError:
                    continue
                labels = {
                    "labor day": "美国劳动节休市",
                    "thanksgiving day": "美国感恩节休市",
                    "christmas day": "美国圣诞节休市",
                    "new year’s day": "美国元旦休市",
                    "new year's day": "美国元旦休市",
                }
                title = labels.get(holiday.lower())
                if title:
                    add(holiday_date, title, "NYSE Holidays")
    ...
    if name == "EIA Weekly Petroleum":
        match = re.search(
            ...
            r"(" + "|".join(
                sorted(
                    {*(m.title() for m in _MONTH_NUM), *(m.title() for m in _MONTH_ABBR_NUM)},
                    key=len,
                    reverse=True,
                )
            ) + r")\.?\s+(\d{1,2}),\s+(\d{4})",
    ...
    # FOMC calendar: within the requested year, each month is followed by the
    # meeting-day range. The policy decision is released on the final day.
    if name == "FOMC Calendar":
        lines = plain.splitlines()
        start_marker = f"{start.year} FOMC Meetings"
        try:
            section_start = next(i for i, line in enumerate(lines) if start_marker.lower() in line.lower())
        except StopIteration:
            section_start = -1
        if section_start >= 0:
            current_month = None
            for line in lines[section_start + 1:]:
                if re.search(r"\b\d{4}\s+FOMC Meetings\b", line, re.I):
                    break
                month_value = line.strip().lower()
                if month_value in _MONTH_NUM:
                    current_month = _MONTH_NUM[month_value]
                    continue
                day_match = re.fullmatch(r"(\d{1,2})(?:\s*-\s*(\d{1,2}))?\*?", line.strip())
                if current_month and day_match:
                    decision_day = int(day_match.group(2) or day_match.group(1))
                    try:
                        meeting_date = date(start.year, current_month, decision_day)
                    except ValueError:
                        continue
                    add(meeting_date, "美联储FOMC利率决议", "Federal Reserve")
                    current_month = None

    # TreasuryDirect upcoming-auction tables expose a named Auction Date
    # column. Keep only medium/long-term note and bond auctions that can move
    # yields; short bill auctions are intentionally omitted.
    if name == "Treasury Auctions Calendar":
        for table in soup.find_all("table"):
            headers = [" ".join(cell.stripped_strings).lower() for cell in table.find_all("th")]
            if not headers:
                first_row = table.find("tr")
                headers = [" ".join(cell.stripped_strings).lower() for cell in first_row.find_all(["th", "td"])] if first_row else []
            try:
                auction_index = next(i for i, value in enumerate(headers) if "auction date" in value)
            except StopIteration:
                continue

            for row in table.find_all("tr"):
                cells = [" ".join(cell.stripped_strings) for cell in row.find_all(["th", "td"])]
                if auction_index >= len(cells):
                    continue
                row_text = " | ".join(cells)
                term_match = re.search(r"\b(2|3|5|7|10|20|30)[ -]?(?:year|yr)\b", row_text, re.I)
                if not term_match:
                    continue
                raw_date = cells[auction_index]
                parsed_date = None
                ...
                if parsed_date:
                    add(
                        parsed_date,
                        f"美国财政部{term_match.group(1)}年期国债拍卖",
                        "Treasury Auctions",
                    )
...
def _normalize_market_calendar_events(ai_events, raw_events, *, start: date, days: int = 14):
    ...
    for item in raw_events or []:
        ...
        title = _specific_company_calendar_title(title, source)
    ...
    # 3) Deduplicate the same event on the same date. A recurring official
    #    release must remain visible on each date inside the 14-day window.
    best_by_title = {}
    for item in candidates:
        key = (item["date"], item["_title_key"])
    ...
    return [
        {
            ...
            "category": x.get("category") or (
                "财报" if "财报" in x["title"] or " ir" in f" {x['source'].lower()}" else "宏观"
            ),
        }
        for x in selected
    ]
...
def _dedicated_calendar_specs(report_date: date):
    ...
    specs = [
        ...
        (
            "New York Fed Indicators",
            f"https://www.newyorkfed.org/research/calendars/i-{report_date.strftime('%b%y').lower()}.html",
            "calendar",
            False,
        ),
        (
            "New York Fed Indicators Next Month",
            f"https://www.newyorkfed.org/research/calendars/i-{next_month_date.strftime('%b%y').lower()}.html",
            "calendar",
            False,
        ),
        (
        ...
        try:
            ...
            if status == "SUCCESS":
                ...
                for event in parsed_events:
                    event.setdefault("url", str(response.url or url))
...
\n请生成中文机构晨报所需的完整 JSON：
{
  ...
  "predictions":[{
    ...
    "sensors":[],"evidence_ids":[]
        ...
        总原则：高信息密度，惜字如金；同时使用普通投资者能一次读懂的专业白话。
...
15. events必须覆盖输入中所有已核实的高影响日程，不得人为只留两三项；同日可列多项，空白日期允许为空。
16. events优先同时覆盖美国政府/美联储与大型公司日程；但每一项都必须有明确日期和真实输入来源，禁止为凑数量编造。
17. 公司日程标题必须直接写公司名和事件类型，例如“英伟达财报”“苹果投资者活动”；禁止“大型科技公司活动”“重点公司财报”等笼统标题。
...
- 若某专有名词在目标语言中没有稳定译名，保留常用英文名，不臆造。
        ...
        direction：
        - title 10-20字，用大众能读懂的专业白话直接说明市场偏强、偏弱或分化；不堆砌“久期、宽基、风险偏好、再定价”等术语。
        - brief 55-85字，固定回答三件事：“现在市场怎么走；为什么；接下来验证什么”。必须使用术语时，紧跟一句白话解释。
        - brief中最多保留3个ticker或缩写，避免用代码串代替结论；brief_en表达完全相同的信息。
...
- focus 最多4个，只留真正相关标的；focus中资产代码不翻译。
        ...
        actions：
        - actions 与 actions_en 的 watch / prepare / avoid 每组最多3项，数量、顺序、标的一一对应。
        - 中文每项18-32字，固定用“标的：动作；触发条件”结构；只保留读者几秒内能执行或监测的信息。
        - 不重复市场定调里的解释，不写背景铺垫；禁止空泛“等待观察”，必须说明等什么。
...
"""
        ...
        prediction_prompt += "\n最近历史预测与真实验证（只用于概率校准，禁止改写历史结论）：\n" + json.dumps(
            self.repository.recent_prediction_reviews(limit=12),
            ensure_ascii=False,
        )
        prediction_prompt += """
...
优先寻找“基本面/事件逻辑未坏，但价格回撤后更有性价比”的股票；绝不能把单纯暴跌当成便宜。
        候选数量不少于8只时必须正好输出8只；只有真实候选不足8只时才允许少于8只。JSON格式：
{"company_signals":[{
...
16. brief 24-36字，只写“核心事实/催化 + 股票含义”；动作全部放入trigger，禁止在brief重复。
17. trigger 14-24字，只写升级/执行所需的一个可观察条件；trigger_en严格对应；禁止空泛“等待进一步确认”；不同公司不得复制同一句触发条件。
18. risk 12-22字，只写一个最可能使当前逻辑失效的条件；risk_en严格对应；不同公司不得复制同一句风险。
...
"""
    ...
    def _limit_company_signals(signals, stock_snapshot, allowed_tickers=None):
        ...
        # 候选行情充足时，用中性、可核验的观察位补足八只。
        # 补位不制造公司新闻或买入观点，只陈述行情并要求新增催化确认。
        selected_tickers = {item["ticker"] for item in selected}
        fallback_order = list(allowed_tickers or stock_snapshot.keys())
        for raw_ticker in fallback_order:
            ticker = str(raw_ticker or "").upper()
            if len(selected) >= final_limit:
                break
            if ticker not in allowed or ticker in selected_tickers:
                continue
            ...
            ma_state = "20日均线上方" if above_ma20 else "20日均线下方"
            brief = (
                f"公司级增量信息不足；现价{price:.2f}，近5日{change_5d:+.2f}%，"
                f"处于{ma_state}，暂不追价。"
            )
            selected.append({
                "company": _company_display_name(ticker),
                ...
                "brief": brief,
                "brief_en": (
                    f"Price {price:.2f}; day {day_change:+.2f}%, five days {change_5d:+.2f}%. "
                    "Company-specific evidence is limited; keep on watch without chasing."
                ),
                "trigger": "放量站稳20日均线，且公司公告或业绩指引提供新增确认。",
                "trigger_en": "Act only after a high-volume hold above the 20-day average and fresh company guidance.",
                "risk": "缺少公司级新增证据；若继续跌破中期均线则取消观察。",
                "risk_en": "Limited company evidence; remove from watch if the medium-term trend breaks further.",
            })
            selected_tickers.add(ticker)

        # 最终展示守门：AI偶尔会为多家公司复用同一句失效风险。
        # 当同文案重复时，根据每只股票的实际价格状态改写为逐股条件，
        # 避免八张卡片机械重复，同时不添加未经输入支持的公司事实。
        risk_counts = {}
        for item in selected:
            risk = " ".join(str(item.get("risk") or "").split())
            if risk:
                risk_counts[risk] = risk_counts.get(risk, 0) + 1

        for item in selected:
            risk = " ".join(str(item.get("risk") or "").split())
            if not risk or risk_counts.get(risk, 0) < 2:
                continue

            ticker = str(item.get("ticker") or "该股").upper()
            snapshot = stock_snapshot.get(ticker, {})
            try:
                day_change = float(snapshot.get("day_change_pct", 0.0))
                change_5d = float(snapshot.get("change_5d_pct", 0.0))
                volatility = float(snapshot.get("volatility_20_pct", 0.0))
                above_ma20 = bool(snapshot.get("above_ma20", False))
            except (TypeError, ValueError):
                day_change = change_5d = volatility = 0.0
                above_ma20 = False

            if volatility >= 3.0:
                item["risk"] = f"{ticker}：放量跌破支撑即失效。"
                item["risk_en"] = f"{ticker} volatility is elevated; invalidate if price breaks support on volume."
            elif change_5d <= -3.0:
                item["risk"] = f"{ticker}：卖压延续并创新低即取消。"
                item["risk_en"] = f"{ticker}'s five-day trend is weak; remove it if selling extends to a new low."
            elif day_change <= -1.0:
                item["risk"] = f"{ticker}：无法止跌并收复均线则不执行。"
                item["risk_en"] = f"{ticker} faces near-term selling; do not act without stabilization and a moving-average reclaim."
            elif above_ma20:
                item["risk"] = f"{ticker}：跌破20日线且缩量即失效。"
                item["risk_en"] = f"Invalidate the {ticker} relative-strength view if it loses the 20-day average on weaker volume."
            else:
                item["risk"] = f"{ticker}：无量反弹后再转弱即取消。"
                item["risk_en"] = f"{ticker} remains below its 20-day average; remove it if a low-volume rebound fails."

            for code in ("zh_tw", "bg", "ru", "ja", "ko", "fr", "de", "es", "th"):
                if code != "en":
                    item.pop(f"risk_{code}", None)

        # 触发条件也必须逐股可执行。若模型或补位逻辑复用同一句，
        # 按该股自身的趋势与波动状态改写，并保留ticker便于快速定位。
        trigger_counts = {}
        for item in selected:
            trigger = " ".join(str(item.get("trigger") or "").split())
            if trigger:
                trigger_counts[trigger] = trigger_counts.get(trigger, 0) + 1

        for item in selected:
            trigger = " ".join(str(item.get("trigger") or "").split())
            if not trigger or trigger_counts.get(trigger, 0) < 2:
                continue
            ...
            if volatility >= 3.0:
                item["trigger"] = f"{ticker}：波动回落且放量收复20日线。"
                item["trigger_en"] = f"Act on {ticker} only after volatility normalizes and price reclaims the 20-day average on volume."
            elif change_5d <= -3.0:
                item["trigger"] = f"{ticker}：止跌并放量收复20日线。"
                item["trigger_en"] = f"Keep {ticker} on hold until it stabilizes and reclaims the 20-day average on volume."
            elif day_change <= -1.0:
                item["trigger"] = f"{ticker}：止跌后获次日量价确认。"
                item["trigger_en"] = f"Raise attention on {ticker} only after stabilization receives next-day price-and-volume confirmation."
            elif above_ma20:
                item["trigger"] = f"{ticker}：回踩20日线不破并重新放量。"
                item["trigger_en"] = f"Consider {ticker} only after a successful 20-day-average retest and renewed volume."
            else:
                item["trigger"] = f"{ticker}：放量站稳20日线且指引确认。"
                item["trigger_en"] = f"Act on {ticker} only after a volume-backed 20-day-average reclaim and fresh company guidance."

            for code in ("zh_tw", "bg", "ru", "ja", "ko", "fr", "de", "es", "th"):
                item.pop(f"trigger_{code}", None)

        for item in selected:
            ticker = str(item.get("ticker") or "").upper()
            item["brief"] = _compact_signal_text(item.get("brief"), max_chars=36)
            item["trigger"] = _compact_signal_text(
                item.get("trigger"), max_chars=28, ticker=ticker
            )
            item["risk"] = _compact_signal_text(
                item.get("risk"), max_chars=26, ticker=ticker
            )
    ...
    def send(self, report: str):
        if self.settings.public_report_url:
            dated_url = (
                ...
                f"/{self.settings.report_date.strftime('%m%d')}.html"
            )
            dated_label = (
            ...
            report = re.sub(
                r"<body([^>]*)>", r"<body\1>" + link, report, count=1, flags=re.I,
            )
