...
def _self_test() -> None:
    ...
    assert calendar.target_session(calendar.base_session(date.today()), 3)
...
def build_parser():
    ...
    parser.add_argument(
        "--check-trading-day",
        action="store_true",
        help="输出报告日期是否为 NYSE 交易日，供定时任务使用",
    )
...
def main(argv=None) -> int:
    ...
    if args.check_trading_day:
        print(str(TradingCalendar().is_session(settings.report_date)).lower())
        return 0
    if args.verify:
        from .db import RadarRepository
        from .live import verify_due_predictions
        count = verify_due_predictions(
            RadarRepository(settings.db_file), today=settings.report_date,
        )
        print(f"完成到期验证: {count}")
        return 0
    offline = os.getenv("NEWS_FINANCE_OFFLINE", "").strip() == "1"
    if offline:
        services = Services(OfflineCollector(), OfflineAnalyzer(), DisabledMailer())
    else:
        missing = settings.validate_runtime(require_ai=True, require_smtp=not args.preview)
        if args.full and not settings.sec_enabled:
        ...
        services = Services(
            HttpCollector(settings), OpenAIAnalyzer(settings), SMTPMailer(settings)
        )

    result = run_pipeline(
        settings,
        services,
        preview=args.preview or offline,
        full=args.full,
    )
