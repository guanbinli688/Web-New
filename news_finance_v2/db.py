...
class RadarRepository:
    ...
    def recent_prediction_reviews(self, *, limit: int = 12, exclude_run_id: str | None = None) -> list[dict]:
        """Return immutable forecast history together with any realized verification."""
        safe_limit = max(1, min(int(limit), 50))
        where = "WHERE p.run_id<>?" if exclude_run_id else ""
        params = (exclude_run_id, safe_limit) if exclude_run_id else (safe_limit,)
        with self.connect() as db:
            rows = db.execute(
                f"""SELECT
                       p.id,p.run_id,p.created_at,p.base_session,p.target_session,
                       p.horizon_days,p.target,p.direction,p.probability,
                       p.thesis,p.invalidation,
                       v.verified_at,v.correct,v.asset_return,v.benchmark_return,
                       v.excess_return,v.max_adverse,v.brier
                    FROM predictions p
                    LEFT JOIN verification v ON v.prediction_id=p.id
                    {where}
                    ORDER BY p.created_at DESC,p.horizon_days,p.target
                    LIMIT ?""",
                params,
            ).fetchall()
