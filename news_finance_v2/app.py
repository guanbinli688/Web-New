...
def run_pipeline(settings: Settings, services: Services, *, preview: bool, full: bool) -> RunSummary:
    ...
    context = {
        **collected, **analysis, "gate": gate, "predictions_frozen": frozen,
        ...
        "prediction_history": repo.recent_prediction_reviews(
            limit=12,
    ...
    metrics = {"market_coverage": collected.get("market_coverage", 0), "frozen": frozen}
    if not preview:
        try:
            services.mailer.send(report)
        except Exception as exc:
            repo.finish_run(run_id, "FAILED", {**metrics, "error": type(exc).__name__})
            raise
    repo.finish_run(run_id, "SUCCESS", metrics)
