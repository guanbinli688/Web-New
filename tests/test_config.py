...
def test_sec_placeholder_is_not_treated_as_real_contact(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("SEC_USER_AGENT", "Your Name your-real-email@example.org")
    assert not Settings.from_env(tmp_path).sec_enabled

    monkeypatch.setenv("SEC_USER_AGENT", "Research Desk analyst@institution.edu")
    assert Settings.from_env(tmp_path).sec_enabled
