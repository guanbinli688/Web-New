...
def test_allowed_prediction_is_frozen_with_trading_day_target(tmp_path):
    ...
    target = BeautifulSoup(result.report_html, "html.parser").select_one("#forecast .forecast-target")
    assert target is not None and target.get_text(strip=True) == "SPY"


def test_email_failure_marks_run_failed(tmp_path):
    with pytest.raises(OSError, match="SMTP unavailable"):
        run_pipeline(
            Settings.from_env(tmp_path),
            Services(Collector(), Analyzer(), FailingMailer()),
            preview=False,
            full=False,
        )

    db = sqlite3.connect(tmp_path / "data" / "news_finance_v2.db")
    status, metrics = db.execute("SELECT status,metrics_json FROM runs").fetchone()
    db.close()
    assert status == "FAILED"
    assert json.loads(metrics)["error"] == "OSError"
