...
def test_is_session_rejects_weekends_and_market_holidays():
    cal = TradingCalendar()
    assert cal.is_session(date(2026, 9, 8))
    assert not cal.is_session(date(2026, 9, 7))
    assert not cal.is_session(date(2026, 9, 12))
