...
def rich_context():
    return {
        ...
        "events": [{"date": "2026-08-20", "title": "初请失业金", "source": "BLS", "category": "宏观"}],
        "sources": [
        ...
        "prediction_history": [{
            "created_at": "2026-08-01T12:00:00+00:00", "target_session": "2026-08-08",
            "horizon_days": 5, "target": "SPY", "direction": "UP", "probability": .62,
            "correct": 1, "asset_return": .021, "excess_return": None, "brier": .144,
        }],
...
def test_report_keeps_v2_layout_with_chinese_labels_and_seven_sections():
    ...
    for text in (
        "GLOBAL MARKET INTELLIGENCE", "NEWS FINANCE",
    ...
    assert 'class="footer-directory"' in report
    assert 'class="footer-brief"' in report
    assert 'class="wrap footer-scope"' not in report
    assert 'class="footer-seal" role="img" aria-label="美国国徽"' in report
    assert 'class="footer-wordmark"' in report
    assert 'class="footer-legal"' in report
    assert 'class="wrap footer-visual"' not in report
    assert 'id="direction"' in report and 'href="#forecast"' in report
    assert "class='capital-map'" in report
    assert "历史验证记录" in report and "命中 · +2.10%" in report
    assert "--canvas:#06152b" in report
    assert "section:nth-of-type(even){background:var(--section-alt)}" in report
    assert "href='https://example.test/bls'" in report
    assert "全球宏观事件驱动投资分析" not in report
    assert "未来14天仅列可能影响主要资产的大事件" not in report
...
def test_calendar_numbers_events_across_all_days_and_names_ir_company():
    context = rich_context()
    context["report_date"] = "2026-09-07"
    context["events"] = [
        {"date": "2026-09-09", "title": "消费者价格指数", "source": "BLS"},
        {"date": "2026-09-09", "title": "生产者价格指数", "source": "BLS"},
        {"date": "2026-09-11", "title": "大型科技公司投资者活动", "source": "NVIDIA IR", "category": "财报"},
    ]

    report = render_report(context)

    assert report.count("<span>01</span>") == 1
    assert report.count("<span>02</span>") == 1
    assert report.count("<span>03</span>") == 1
    assert "英伟达投资者活动" in report
    assert "大型科技公司投资者活动" not in report
